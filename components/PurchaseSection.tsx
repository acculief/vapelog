...
export default function PurchaseSection({ productName, price, affiliates, avgRating, reviewCount }: Props) {
  ...
  const isDirect = amazonUrl.includes('/dp/')
  const ctaText = isDirect ? '今すぐAmazonで購入する' : 'Amazonで最安値をチェック'
  ...
  return (
    <>
      ...
      <div className="rounded-2xl overflow-hidden mb-8 border border-yellow-500/30" id="purchase-section">
        {/* Header bar */}
        <div className="px-5 py-3 flex items-center justify-between" style={{ background: 'linear-gradient(90deg, rgba(124,58,237,0.25), rgba(79,70,229,0.15))' }}>
          <div className="flex items-center gap-2">
            <span className="text-yellow-400 font-black text-sm">🏷️ 購入する</span>
            {isDirect && (
              <span className="text-xs bg-green-500/20 text-green-400 border border-green-500/30 px-2 py-0.5 rounded-full">Amazon直リンク</span>
            )}
          </div>
          {reviewCount > 0 && (
            <div className="flex items-center gap-1 text-sm">
              ...
              <span className="text-gray-400 text-xs">({reviewCount}件)</span>
            </div>
          )}
        </div>

        <div className="p-5" style={{ background: 'rgba(0,0,0,0.35)' }}>
          {/* Price display */}
          {price && (
            <div className="flex items-baseline gap-2 mb-4">
              ...
              <span className="text-xs text-gray-500">（税込・Amazon価格は変動あり）</span>
            </div>
          )}
          ...
          <a
            ...
            className="group block w-full text-center font-black text-lg py-4 px-6 rounded-xl transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] mb-3"
            style={{
          ...
          >
            <div className="flex items-center justify-center gap-3">
              ...
              <div className="text-left">
                ...
                <div className="text-xs font-normal opacity-70 mt-0.5">✓ Prime会員なら翌日配送　✓ 返品・交換OK</div>
              </div>
              {price && (
                <span className="ml-auto text-base font-black border-l border-black/20 pl-3">
            ...
            <div className="flex gap-2 mb-4">
              {affiliates?.rakuten && (
                <a href={affiliates.rakuten} target="_blank" rel="noopener noreferrer sponsored"
                  className="flex-1 text-center bg-red-600 hover:bg-red-500 text-white font-bold py-3 rounded-xl transition text-sm flex items-center justify-center gap-1">
                  <span>🏬</span> 楽天市場で購入
                </a>
              )}
              {affiliates?.yahoo && (
                <a href={affiliates.yahoo} target="_blank" rel="noopener noreferrer sponsored"
                  className="flex-1 text-center bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 rounded-xl transition text-sm flex items-center justify-center gap-1">
                  <span>🛒</span> Yahoo!ショッピング
                </a>
              )}
            </div>
          )}
          ...
          {/* Trust badges */}
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-400 pt-3 border-t border-white/10">
            <span>🔒 SSL暗号化通信</span>
            <span>📦 最短翌日お届け</span>
            <span>↩️ 返品・交換保証</span>
            <span>✅ 正規品保証</span>
      ...
      {/* Sticky bottom bar (shows after scrolling past main section) */}
      <div
